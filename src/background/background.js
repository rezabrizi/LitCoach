      ...
      async ([{ id, url }]) => {
        ...
        const [result] = await chrome.scripting.executeScript({
          ...
          func: () => {
            const isPremium =
              document
                .querySelector(".text-brand-orange")
                ?.textContent.trim() === "Premium";
            return window.monaco.editor
              .getModels()
              [isPremium ? 0 : 1].getValue();
          },
        });
        sendResponse(
          result?.result
  ...
  if (message.action === "github_auth") {
    ...
    chrome.identity.launchWebAuthFlow(
      ...
      async (responseUrl) => {
        ...
        await axios
          .post(
            `${API_URL}/github_access_token`,
            JSON.stringify({ code: code.toString() }),
            {
              headers: {
