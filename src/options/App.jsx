...
const App = () => {
    ...
    const fetchUserData = async () => {
      const data = await new Promise((resolve) => {
        chrome.storage.sync.get(
          ["user_data", "user_id", "selected_repo_id", "sync_enabled"],
          resolve
        );
      });
  ...
  const handleRepoSelect = async (repoId) => {
    const selected = userData.repos.find(
      ...
      <div className='space-y-4 flex flex-col w-full max-w-md'>
        <Card>
          ...
          {syncEnabled && (
            <CardContent>
              <Tabs
                defaultValue={userData.repos.length > 0 ? "select" : "create"}
              >
                ...
                <TabsContent value='create' className='mt-4'>
                  <Form {...form}>
                    <form
                      onSubmit={form.handleSubmit(handleCreateRepo)}
                      className='space-y-4'
                    >
                      <FormField
